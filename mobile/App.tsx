...
const API_URL = process.env.EXPO_PUBLIC_API_URL || 'http://127.0.0.1:8004';
...
function PublicToursScreen() {
  const [tours, setTours] = useState<any[]>([]);
  const [q, setQ] = useState('');
  const load = async () => {
    const url = new URL(`${API_URL}/public/tours`);
  ...
  return (
    <ScrollView style={{ flex: 1, padding: 16 }}>
      <Text style={{ fontSize: 20, fontWeight: 'bold', marginBottom: 12 }}>Витрина — Туры</Text>
      <View style={{ flexDirection: 'row', marginBottom: 12 }}>
        <TextInput value={q} onChangeText={setQ} placeholder="Поиск" style={{ flex: 1, borderWidth: 1, borderColor: '#ddd', borderRadius: 8, padding: 10, marginRight: 8 }} />
        <TouchableOpacity onPress={load} style={{ backgroundColor: '#3498db', padding: 12, borderRadius: 8 }}>
      ...
      {tours.map((t) => (
        <View key={t.id} style={{ backgroundColor: '#fff', padding: 12, borderRadius: 10, marginBottom: 10, borderWidth: 1, borderColor: '#eee' }}>
          ...
          <Text style={{ color: '#7f8c8d', marginTop: 4 }}>{t.price} {t.currency} • {t.duration_hours ? `${t.duration_hours} ч.` : 'длительность не указана'}{t.difficulty ? ` • ${t.difficulty}` : ''}</Text>
          <Text style={{ color: '#7f8c8d', marginTop: 4 }}>{t.location?.city ? `Город: ${t.location.city}` : ''}</Text>
        </View>
      ))}
    </ScrollView>
  );
...
function PublicActivitiesScreen() {
  const [items, setItems] = useState<any[]>([]);
  const [q, setQ] = useState('');
  const load = async () => {
    const url = new URL(`${API_URL}/public/activities`);
    if (q.trim()) url.searchParams.set('q', q.trim());
    const res = await fetch(url.toString());
  ...
  return (
    <ScrollView style={{ flex: 1, padding: 16 }}>
      <Text style={{ fontSize: 20, fontWeight: 'bold', marginBottom: 12 }}>Витрина — Активности</Text>
      <View style={{ flexDirection: 'row', marginBottom: 12 }}>
        <TextInput value={q} onChangeText={setQ} placeholder="Поиск" style={{ flex: 1, borderWidth: 1, borderColor: '#ddd', borderRadius: 8, padding: 10, marginRight: 8 }} />
        <TouchableOpacity onPress={load} style={{ backgroundColor: '#3498db', padding: 12, borderRadius: 8 }}>
          <Text style={{ color: '#fff' }}>Найти</Text>
      ...
      {items.map((a) => (
        <View key={a.id} style={{ backgroundColor: '#fff', padding: 12, borderRadius: 10, marginBottom: 10, borderWidth: 1, borderColor: '#eee' }}>
          ...
          <Text style={{ color: '#7f8c8d', marginTop: 4 }}>{a.price ? `${a.price} ₽` : 'Цена по запросу'}{a.duration_hours ? ` • ${a.duration_hours} ч.` : ''}</Text>
          <Text style={{ color: '#7f8c8d', marginTop: 4 }}>{a.tags?.length ? `Теги: ${a.tags.join(', ')}` : ''}</Text>
        </View>
      ))}
    </ScrollView>
  );
...
function StorefrontScreen() {
  const [tab, setTab] = useState<'tours'|'activities'>('tours');
  return (
    <View style={{ flex: 1 }}>
      <View style={{ flexDirection: 'row', padding: 12, gap: 8 }}>
        <TouchableOpacity onPress={() => setTab('tours')} style={{ flex: 1, backgroundColor: tab==='tours' ? '#3498db' : '#ecf0f1', padding: 10, borderRadius: 8, alignItems: 'center' }}>
          <Text style={{ color: tab==='tours' ? '#fff' : '#2c3e50' }}>Туры</Text>
        </TouchableOpacity>
        <TouchableOpacity onPress={() => setTab('activities')} style={{ flex: 1, backgroundColor: tab==='activities' ? '#3498db' : '#ecf0f1', padding: 10, borderRadius: 8, alignItems: 'center' }}>
          <Text style={{ color: tab==='activities' ? '#fff' : '#2c3e50' }}>Активности</Text>
        </TouchableOpacity>
      </View>
      {tab === 'tours' ? <PublicToursScreen /> : <PublicActivitiesScreen />}
    </View>
  );
}
...
function OperatorTabs() {
  return (
    <Tabs.Navigator screenOptions={{ headerShown: false }}>
      <Tabs.Screen name="Storefront" component={StorefrontScreen} options={{ title: 'Витрина', tabBarIcon: ({ color, size }) => <MaterialIcons name="storefront" color={color} size={size} /> }} />
      <Tabs.Screen name="Tours" component={OperatorToursScreen} options={{ title: 'Мои туры', tabBarIcon: ({ color, size }) => <MaterialIcons name="inventory" color={color} size={size} /> }} />
    </Tabs.Navigator>
  );
}
...
function GuideTabs() {
  return (
    <Tabs.Navigator screenOptions={{ headerShown: false }}>
      <Tabs.Screen name="Storefront" component={StorefrontScreen} options={{ title: 'Витрина', tabBarIcon: ({ color, size }) => <MaterialIcons name="storefront" color={color} size={size} /> }} />
      <Tabs.Screen name="Activities" component={GuideActivitiesScreen} options={{ title: 'Мои активности', tabBarIcon: ({ color, size }) => <MaterialIcons name="hiking" color={color} size={size} /> }} />
    </Tabs.Navigator>
  );
}
